...
<Callout type="info">
  When the store file tracking implementation is specified in **hbase\\_site.xml**, this configuration is also propagated into a tables configuration at table creation time. This is to avoid dangerous configuration mismatches between processes, which
...
commit is delegated to the specific Store File Tracking implementation.
The implementation can be set at the HBase service leve in hbase-site.xml or at the
Table or Column Family via the TableDescriptor configuration.`},{heading:"store-file-tracking-overview",content:"type: info"},{heading:"store-file-tracking-overview",content:`When the store file tracking implementation is specified in hbase_site.xml, this configuration is also propagated into a tables configuration at table creation time. This is to avoid dangerous configuration mismatches between processes, which
could potentially lead to data loss.`},{heading:"available-implementations",content:"Store File Tracking initial version provides three builtin implementations:"},{heading:"available-implementations",content:"DEFAULT"},{heading:"available-implementations",content:"FILE"},{heading:"available-implementations",content:"MIGRATION"},{heading:"default",content:`As per the name, this is the Store File Tracking implementation used by default when no explicit
configuration has been defined. The DEFAULT tracker implements the standard approach using temporary
...
feature, the Store File Tracking implementation can only be changed with the MIGRATION
implementation, so that the new tracker can safely build its list of tracked files based on the
list of the current tracker.`},{heading:"store-file-tracking-usage",content:"type: info"},{heading:"store-file-tracking-usage",content:`MIGRATION tracker should NOT be set at global configuration. To use it, follow below section
about setting Store File Tacking at Table or Column Family configuration.`},{heading:"configuring-for-table-or-column-family",content:`Setting Store File Tracking configuration globally may not always be possible or desired, for example,
in the case of upgraded clusters with pre-existing user data.
Store File Tracking can be set at Table or Column Family level configuration.
For example, to specify FILE implementation in the table configuration at table creation time,
...
Table or Column Family via the TableDescriptor configuration.`]}),`
`,e.jsx(n,{type:"info",children:e.jsxs(i.p,{children:["When the store file tracking implementation is specified in ",e.jsx(i.strong,{children:"hbase_site.xml"}),`, this configuration is also propagated into a tables configuration at table creation time. This is to avoid dangerous configuration mismatches between processes, which
could potentially lead to data loss.`]})}),`
...
`,e.jsx(n,{type:"info",children:e.jsx(i.p,{children:`MIGRATION tracker should NOT be set at global configuration. To use it, follow below section
about setting Store File Tacking at Table or Column Family configuration.`})}),`
