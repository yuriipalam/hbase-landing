...
`,e.jsx(s,{type:"info",title:"Running in MapReduce mode",children:e.jsxs(i.p,{children:["MapReduce mode offers the ability to process each family dir in parallel, as a separate map task. Generally, it would make sense to run in this mode when specifying one or more table dirs as targets for compactions. The caveat, though, is that if number of families to be compacted become too large, the related mapreduce job may have indirect impacts on ",e.jsx(i.strong,{children:"RegionServers"})," performance . Since ",e.jsx(i.strong,{children:"NodeManagers"})," are normally co-located with RegionServers, such large jobs could compete for IO/Bandwidth resources with the ",e.jsx(i.strong,{children:"RegionServers"}),"."]})}),`
`,e.jsx(s,{type:"info",title:"MajorCompaction completely disabled on RegionServers due performance impacts",children:e.jsxs(i.p,{children:[e.jsx(i.strong,{children:"Major compactions"})," can be a costly operation (see ",e.jsx(i.a,{href:"/docs/architecture/regions#compaction",children:"compaction"}),"), and can indeed impact performance on RegionServers, leading operators to completely disable it for critical low latency application. ",e.jsx(i.strong,{children:"CompactionTool"})," could be used as an alternative in such scenarios, although, additional custom application logic would need to be implemented, such as deciding scheduling and selection of tables/regions/families target for a given compaction run."]})}),`
