...
<Callout type="info">
  Note the differentiation between the \`hbase backup set add\` command and the *-set* option. The \`hbase backup set add\`
...
and its options. The below information is captured in this help message for each command.`},{heading:"creating-a-backup-image",content:"type: info"},{heading:"creating-a-backup-image",content:`For HBase clusters also using Apache Phoenix: include the SQL system catalog tables in the backup. In the event that you
need to restore the HBase backup, access to the system catalog tables enable you to resume Phoenix interoperability with the
restored data.`},{heading:"creating-a-backup-image",content:`The first step in running the backup and restore utilities is to perform a full backup and to store the data in a separate image
...
The backup ID is the Unix time (also known as Epoch time) that the HBase master received the backup request from the client.`},{heading:"creating-a-backup-image",content:"type: info"},{heading:"creating-a-backup-image",content:`Record the backup ID that appears at the end of a successful backup. In case the source cluster fails and you need to recover the
dataset with a restore operation, having the backup ID readily available can save time.`},{heading:"backup-restore-commands-creating-a-backup-image-positional-command-line-arguments",content:`typeThe type of backup to execute: full or incremental. As a reminder, an incremental backup requires a full backup to
...
• /tmp/backup_incremental is the path to the directory containing the backup image.
• backupId_1467823988425 is the backup ID.
• mytable1 and mytable2 are the names of tables in the backup image to be restored.`},{heading:"example-of-usage",content:"type: info"},{heading:"example-of-usage",content:`If the namespace of a table being restored does not exist in the target environment, it will be automatically created during the restore operation.
...
of table names. You can group tables into a named backup set with the hbase backup set add command. You can then use
the -set option to invoke the name of a backup set in the hbase backup create or hbase restore rather than list
individually every table in the group. You can have multiple backup sets.`},{heading:"using-backup-sets",content:"type: info"},{heading:"using-backup-sets",content:`Note the differentiation between the hbase backup set add command and the -set option. The hbase backup set add
command must be run before using the -set option in a different command because backup sets must be named and defined
before using backup sets as a shortcut.`},{heading:"using-backup-sets",content:`If you run the hbase backup set add command and specify a backup set name that does not yet exist on your system, a new set
is created. If you run the command with the name of an existing backup set name, then the tables that you specify are added
to the set.`},{heading:"using-backup-sets",content:"In this command, the backup set name is case-sensitive."},{heading:"using-backup-sets",content:"type: info"},{heading:"using-backup-sets",content:`The metadata of backup sets are stored within HBase. If you do not have access to the original HBase cluster with the
backup set metadata, then you must specify individual table names to restore the data.`},{heading:"using-backup-sets",content:"To create a backup set, run the following command as the HBase superuser:"},{heading:"backup-set-subcommands",content:"The following list details subcommands of the hbase backup set command."},{heading:"backup-set-subcommands",content:"type: info"},{heading:"backup-set-subcommands",content:`You must enter one (and no more than one) of the following subcommands after hbase backup set to complete an operation.
Also, the backup set name is case-sensitive in the command-line utility.`},{heading:"backup-set-subcommands",content:"addAdds table[s] to a backup set. Specify a backup_set_name value after this argument to create a backup set."},{heading:"backup-set-subcommands",content:"removeRemoves tables from the set. Specify the tables to remove in the tables argument."},{heading:"backup-set-subcommands",content:"listLists all backup sets."},{heading:"backup-set-subcommands",content:`describeDisplays a description of a backup set. The information includes whether the set has full
or incremental backups, start and end times of the backups, and a list of the tables in the set. This subcommand must precede
...
`,e.jsx(t,{type:"info",children:e.jsx(a.p,{children:`For HBase clusters also using Apache Phoenix: include the SQL system catalog tables in the backup. In the event that you
need to restore the HBase backup, access to the system catalog tables enable you to resume Phoenix interoperability with the
restored data.`})}),`
...
The backup ID is the Unix time (also known as Epoch time) that the HBase master received the backup request from the client.`]}),`
`,e.jsx(t,{type:"info",children:e.jsx(a.p,{children:`Record the backup ID that appears at the end of a successful backup. In case the source cluster fails and you need to recover the
dataset with a restore operation, having the backup ID readily available can save time.`})}),`
...
`,e.jsx(t,{type:"info",children:e.jsxs(a.p,{children:[`If the namespace of a table being restored does not exist in the target environment, it will be automatically created during the restore operation.
...
individually every table in the group. You can have multiple backup sets.`]}),`
`,e.jsx(t,{type:"info",children:e.jsxs(a.p,{children:["Note the differentiation between the ",e.jsx(a.code,{children:"hbase backup set add"})," command and the ",e.jsx(a.em,{children:"-set"})," option. The ",e.jsx(a.code,{children:"hbase backup set add"}),`
command must be run before using the `,e.jsx(a.code,{children:"-set"}),` option in a different command because backup sets must be named and defined
before using backup sets as a shortcut.`]})}),`
...
`,e.jsx(t,{type:"info",children:e.jsx(a.p,{children:`The metadata of backup sets are stored within HBase. If you do not have access to the original HBase cluster with the
backup set metadata, then you must specify individual table names to restore the data.`})}),`
...
`,e.jsx(t,{type:"info",children:e.jsx(a.p,{children:`You must enter one (and no more than one) of the following subcommands after hbase backup set to complete an operation.
Also, the backup set name is case-sensitive in the command-line utility.`})}),`
...
`,`List of tables (or a single table) to include in the backup set. Enter the table names as a comma-separated list. If no tables
are specified, all tables are included in the set.`]}),`
`,e.jsx(t,{type:"info",children:e.jsx(a.p,{children:`Maintain a log or other record of the case-sensitive backup set names and the corresponding tables in each set on a separate
or remote cluster, backup strategy. This information can help you in case of failure on the primary cluster.`})}),`
