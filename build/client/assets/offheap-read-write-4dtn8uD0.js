...
Please remember to adjust value of 'HBASE\\_OFFHEAPSIZE' in *hbase-env.sh* (See [BucketCache Example Configuration](/docs/architecture/regionserver#bucketcache-example-configuration) for help sizing and an example
...
more about hbase.bucketcache.ioengine options). Also specify the total capacity of the BC using hbase.bucketcache.size.
Please remember to adjust value of 'HBASE_OFFHEAPSIZE' in hbase-env.sh (See BucketCache Example Configuration for help sizing and an example
enabling). This configuration is for specifying the maximum possible off-heap memory allocation for the RegionServer java
process. This should be bigger than the off-heap BC size to accommodate usage by other features making use of off-heap memory
...
make a result cell block. This can impact the GC on a highly read loaded server.`},{heading:"tuning-the-rpc-buffer-pool",content:"type: info"},{heading:"tuning-the-rpc-buffer-pool",content:`The config keys which start with prefix hbase.ipc.server.reservoir are deprecated in hbase-3.x (the
internal pool implementation changed). If you are still in hbase-2.2.x or older, then just use the old config
keys. Otherwise if in hbase-3.x or hbase-2.3.x+, please use the new config keys
(See deprecated and new configs in HBase3.x)`},{heading:"tuning-the-rpc-buffer-pool",content:`Next thing to tune is the ByteBuffer pool on the RPC server side. The user can tune this pool with respect to how
many buffers are in the pool and what should be the size of each ByteBuffer. Use the config
hbase.ipc.server.reservoir.initial.buffer.size to tune each of the buffer sizes. Default is 64KB for hbase-2.2.x
and less, changed to 65KB by default for hbase-2.3.x+
...
Please remember to adjust value of 'HBASE_OFFHEAPSIZE' in `,e.jsx(a.em,{children:"hbase-env.sh"})," (See ",e.jsx(a.a,{href:"/docs/architecture/regionserver#bucketcache-example-configuration",children:"BucketCache Example Configuration"}),` for help sizing and an example
enabling). This configuration is for specifying the maximum possible off-heap memory allocation for the RegionServer java
process. This should be bigger than the off-heap BC size to accommodate usage by other features making use of off-heap memory
...
make a result cell block. This can impact the GC on a highly read loaded server.`}),`
`,e.jsx(t,{type:"info",children:e.jsxs(a.p,{children:["The config keys which start with prefix ",e.jsx(a.code,{children:"hbase.ipc.server.reservoir"}),` are deprecated in hbase-3.x (the
internal pool implementation changed). If you are still in hbase-2.2.x or older, then just use the old config
keys. Otherwise if in hbase-3.x or hbase-2.3.x+, please use the new config keys
(See `,e.jsx(a.a,{href:"/docs/offheap-read-write#read-block-from-hdfs-to-offheap-directly",children:"deprecated and new configs in HBase3.x"}),")"]})}),`
